...
exports.run = (client, message, args) => {
  try {
    ...
    sql.get(`SELECT * FROM profiles WHERE guildId ="${message.guild.id}" AND userId ="${message.author.id}"`).then(row => {
	  ...
	  if (!row) return message.channel.nend("Necesitas hablar primero.")
	  if (row.cash < 0) return message.channel.send("No tienes dinero.")
      ...
      if (coinflip === 'Heads!' && headsortails === "heads") {
        ...
        message.channel.send("That option wasnt found, command usage kn!flipcoin [bet] [heads/tails]")
      }
    })
  } catch (err) {
    console.log(err)
