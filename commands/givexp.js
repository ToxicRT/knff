...
exports.run = (client, message, args) => {
    ...
    const user = message.mentions.users.first();
     if (message.mentions.users.size < 1) {
        ...
        if (amount >= 9999999999999999999999999999999999) return message.channel.send("Max xp to give is 9999999999999999999999999999999999")
        sql.get(`SELECT * FROM profiles WHERE guildId ="${message.guild.id}" AND userId ="${message.author.id}"`).then(row2 => {
            ...
            if (row2.xp >= 9999999999999999999999999999999999) return message.channel.send("Max xp is 9999999999999999999999999999999999")
    ...
    sql.get(`SELECT * FROM profiles WHERE guildId ="${message.guild.id}" AND userId ="${user.id}"`).then(row => {
     ...
     if (row.xp >= 9999999999999999999999999999999999) return message.channel.send("Max xp is 9999999999999999999999999999999999")
        if (amount >= 9999999999999999999999999999999999) return message.channel.send("Max xp to give is 9999999999999999999999999999999999")
        if (isNaN(amount)) return message.channel.send("Not a valid number")
