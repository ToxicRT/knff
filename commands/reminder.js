...
exports.run = (client, message, args) => {
  ...
  if (messagez.length < 1) return message.channel.send('Formato incorrecto. k-reminder <minutos> <mensaje>');
  return new Promise((resolve) => {
    if (!isNaN(messagez[0])) {
      ...
      setTimeout(() => {
        message.reply(`Recuerda: ${messagez.split(' ').slice(1).join(' ')}!`);
      }, time * 60000);
    ...
    return message.channel.send(`Te lo recordare en ${minutes} ${minutemessage} for ${messagez}.`);
  });
}
